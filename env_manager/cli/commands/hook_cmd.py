...
def hook(
    install: bool = typer.Option(False, "--install", help="Generate hook script for your shell"),
    uninstall: bool = typer.Option(False, "--uninstall", help="Remove hook from shell config"),
    ping: str = typer.Option(None, "--ping", hidden=True, help="Internal: notify daemon of cd"),
) -> None:
    ...
    if install:
        ...
        typer.echo("Shell hooks auto-detect environments when you cd into a project.")
        typer.echo("")
        typer.echo("  envs hook --install     Generate and show the hook script")
        typer.echo("  envs hook --uninstall   Remove hook from shell config")
        typer.echo("")
        typer.echo("Hooks are opt-in. We never modify your shell config automatically.")
...
def _do_install() -> None:
    ...
    typer.echo("")
    typer.echo("Add the following to your shell config to enable auto-detection on cd:")
    typer.echo("")
    typer.echo("```bash")
    typer.echo(HOOK_SCRIPT.strip())
    typer.echo("```")
    typer.echo("")
    typer.echo(f"To add automatically, run: envs hook --install | tee -a {rc_file}")
...
def _do_uninstall() -> None:
    ...
    typer.echo("")
    typer.echo(f"Or run: grep -v 'env-manager' {rc_file} > {rc_file}.tmp && mv {rc_file}.tmp {rc_file}")
